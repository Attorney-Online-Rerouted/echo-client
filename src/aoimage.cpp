    ...
    : QLabel(parent)
{
  ...
  if (!is_static) // Only create the QMovie if we're non-static
  {
    ...
    connect(movie, &QMovie::frameChanged, [this] {
      ...
      f_pixmap = f_pixmap.scaled(this->size(), Qt::IgnoreAspectRatio);
...
{
  QString p_image_resolved = ao_app->get_image(p_image, Options::getInstance().theme(), Options::getInstance().subTheme(), ao_app->default_theme, p_misc, "", "", is_static || !Options::getInstance().animatedThemeEnabled());
