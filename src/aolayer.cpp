...
void AOLayer::move_and_center(int ax, int ay)
{
  ...
  {
    center_pixmap(movie_frames[0]); // just use the first frame since dimensions
                                    // are all that matter
  }
}
...
void CharLayer::load_image(QString p_filename, QString p_charname, int p_duration, bool p_is_preanim)
{
  ...
  QVector<VPath> pathlist{// cursed character path resolution vector
                          ao_app->get_character_path(p_charname,
                                                     prefix + current_emote), // Default path
                          ao_app->get_character_path(p_charname,
...
void EffectLayer::load_image(QString p_filename, bool p_looping)
{
  ...
  start_playback(p_filename); // path resolution is handled by the caller for
                              // EffectLayer objects
  play();
}
...
void AOLayer::start_playback(QString p_image)
{
  ...
  {
    frameAdded.wait(&mutex); // wait for the frame loader to add another frame,
                             // then check again
  }
...
#ifdef DEBUG_MOVIE
    qDebug() << "[AOLayer::start_playback] max_frames is <= 1, using static "
                "duration";
#endif
  }
...
void CharLayer::play_frame_effect(int p_frame)
{
  ...
  {
    qWarning() << "Attempted to play a frame effect bigger than the size of "
                  "movie_effects";
    return;
  }
  if (p_frame < max_frames)
  {
    foreach (QString effect, movie_effects[p_frame])
    {
      if (effect == "shake")
      {
        emit shake();
#ifdef DEBUG_MOVIE
        qDebug() << "[CharLayer::play_frame_effect] Attempting to play shake "
                    "on frame"
                 << frame;
#endif
      }
      ...
      if (effect == "flash")
      {
        emit flash();
#ifdef DEBUG_MOVIE
        qDebug() << "[CharLayer::play_frame_effect] Attempting to play flash "
                    "on frame"
                 << frame;
#endif
      }
