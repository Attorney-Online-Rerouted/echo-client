...
QString AOMusicPlayer::play(QString p_song, int channel, bool loop, int effect_flags)
{
  ...
  if (loop && file_exists(d_path)) // Contains loop/etc. information file
  {
    ...
    foreach (QString line, lines)
    {
      ...
      QWORD bytes = static_cast<QWORD>(args[1].trimmed().toUInt() * sample_size * num_channels);
      if (arg == "loop_start")
      {
    ...
    qDebug() << "Found data file for song" << p_song << "length" << BASS_ChannelGetLength(newstream, BASS_POS_BYTE) << "loop start" << loop_start[channel] << "loop end" << loop_end[channel];
  }
...
void AOMusicPlayer::set_looping(bool loop_song, int channel)
{
  ...
  if (loop_start[channel] < loop_end[channel])
  {
    ...
    loop_sync[channel] = BASS_ChannelSetSync(m_stream_list[channel], BASS_SYNC_POS | BASS_SYNC_MIXTIME, loop_end[channel], loopProc, &loop_start[channel]);
  }
  else
  {
    // Loop when the end of the file is reached.
    loop_sync[channel] = BASS_ChannelSetSync(m_stream_list[channel], BASS_SYNC_END | BASS_SYNC_MIXTIME, 0, loopProc, &loop_start[channel]);
  }
}
