...
void Courtroom::set_char_select_page()
{
  ...
  char_columns = ((ui_char_buttons->width() - button_width) / (f_spacing.x() + button_width)) + 1;
  char_rows = ((ui_char_buttons->height() - button_height) / (f_spacing.y() + button_height)) + 1;
...
void Courtroom::char_clicked(int n_char)
{
  ...
  if (n_char != m_cid || n_char == -1)
  {
    ...
    ao_app->send_server_packet(new AOPacket("CC", {QString::number(ao_app->client_id), QString::number(n_char), get_hdid()}));
  }
  if (n_char == m_cid || n_char == -1)
  {
...
void Courtroom::character_loading_finished()
{
  ...
  for (int n = 0; n < char_list.size(); n++)
  {
    ...
    treeItem->setIcon(0, QIcon(ao_app->get_image_suffix(ao_app->get_character_path(char_list.at(n).name, "char_icon"))));
    ...
    connect(char_button, &AOCharButton::clicked, [this, n]() { this->char_clicked(n); });
