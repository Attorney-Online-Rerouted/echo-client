...
void Courtroom::set_font(QWidget *widget, QString class_name, QString p_identifier, QString p_char, QString font_name, int f_pointsize)
{
  ...
  if (f_pointsize <= 0)
  {
    f_pointsize = ao_app->get_design_element(p_identifier, design_file, ao_app->get_chat(p_char)).toInt();
  }
  if (font_name == "")
  {
...
{
  ui_debug_log->append_chatmessage(QString(), f_message, ao_app->get_color("debug_log_color", "courtroom_fonts.ini").name());
}
...
void Courtroom::regenerate_ic_chatlog()
{
  ...
  foreach (chatlogpiece item, ic_chatlog_history)
  {
    ...
    QString name = ui_showname_enable->isChecked() ? item.get_showname() : item.get_name();
    append_ic_text(message, name, item.get_action(), item.get_chat_color(), item.get_selfname(), item.get_datetime().toLocalTime());
  }
}
